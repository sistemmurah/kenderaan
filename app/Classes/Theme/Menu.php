...
class Menu
{
     ...
     * @return string
     */
    public static function renderVerMenu($item, $menus, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['code'])) {
            // foreach (Cache::get('menus'.Auth::id().session('current_role')) as $menu) {
                if ($item['code'] == $menus['code']) {
                    if (isset($item['separator'])) {
                        echo '<li class="menu-separator"><span></span></li>';
                    } elseif (isset($item['section'])) {
                        echo '<li class="menu-section ' . ($rec === 0 ? 'menu-section--first' : '') . '">
                            <h4 class="menu-text text-white"> Menu ' . $menus['value'] . '</h4>
                            <i class="menu-icon flaticon-more-v2"></i>
                        </li>';                        
                    }
                } 
                ...
                $check_sub_warga = '';
                // $check_sub_others = '';
                if (isset($menus['submenu']) && isset($menus['id'])) {
                    $check_sub_warga = substr($menus['submenu'][$menus['id']]['code'], 0, -4);
                }

                if (($item['code'] == $menus['code'] || $item['code'] == $check_sub_warga) && $menus['code'] != 'section') {
                    if (isset($item['title'])) {
                            $item_class = '';
                            $item_attr = '';
                            ...
                            if (isset($item['submenu'])) {
                                ...
                                if (isset($item['root'])) {
                                    ...
                                    unset($parent_item['icon']);
                                    unset($parent_item['submenu']);
                                    self::renderVerMenu($parent_item, $menus, null, $rec++, true); // single item render
                                }

                                //utk loop child
                                if (isset($menus['submenu'])) {
                                    foreach ($menus['submenu'] as $sub) {
                                        foreach ($item['submenu'] as $submenu_item) {
                                            if ($submenu_item['code'] == $sub['code']) {
                                                self::renderVerMenu($submenu_item, $sub, $item, $rec++);
                                            }
                                        }
                                    }
                                }
            ...
            if (is_array($item) || is_object($item) || is_array($menus) || is_object($menus)) {
                
                // dd($menus);
                foreach($menus as $menu) {
                    foreach ($item as $each) {
                        self::renderVerMenu($each, $menu, $parent, $rec++);
                    }
                }
    ...
    public static function renderIcon($icon)
    {
        ...
        if (Metronic::isSVG($icon)) {
             echo Metronic::getSVG($icon, 'menu-icon svg-icon-primary');
        } else {
            echo '<i class="svg-icon '.$icon.' svg-icon-primary"></i>';
        }
